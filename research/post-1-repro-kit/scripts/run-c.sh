...
while IFS=$'\t' read -r repo tier vendors stars pushed branch cfgs; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$repo" "$tier" "$vendors" "$fw" "$wraw" "$fpraw" "$dflag" "$dlogin" "$ddep" "$eflag" "$ekind" "$fkind" "$gflag" "$hflag" "$ntest" "$nauth" "$stars" "$pushed" >> "$D/fields-c.tsv"
  cd "$D" || exit
done < "$D/part-c"
echo "MEASURED: $(wc -l < "$D/fields-c.tsv")" >&2
