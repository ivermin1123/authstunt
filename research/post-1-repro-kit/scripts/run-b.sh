...
while IFS=$'\t' read -r repo tier vendors stars pushed branch cfgs; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$repo" "$tier" "$vendors" "$fw" "$wraw" "$fpraw" "$dflag" "$dlogin" "$ddep" "$eflag" "$ekind" "$fkind" "$gflag" "$hflag" "$ntest" "$nauth" "$stars" "$pushed" >> "$D/fields-b.tsv"
  cd "$D" || exit
done < "$D/part-b"
echo "MEASURED: $(wc -l < "$D/fields-b.tsv")" >&2
